...
# Sync agent definitions to MinIO
...
- **`agents`** — Agent definitions (name, model, tags)
- **`instructions`** — Skills and system prompts (`is_skill` boolean distinguishes them)
...
- **`decapod_state`** — Per-session state including full `message_history` and `tools` as JSONB
...
Skills are Markdown files with YAML frontmatter and a `specs:` JSON block defining parameters. The `2_skills.yaml` file for each agent lists which skills are active.
